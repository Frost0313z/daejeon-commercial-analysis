...
6. [분석 결과](#6-분석-결과)
   ...
   - [6.8 점포 교체율: 진입·이탈 분해](#68-점포-교체율-진입이탈-분해)
   - [6.9 인터랙티브 대시보드](#69-인터랙티브-대시보드)
7. [주요 인사이트](#7-주요-인사이트)
8. [결론](#8-결론)
9. [한계점](#9-한계점)
10. [재현 방법](#10-재현-방법)
...
증감률과 잔존율만으로는 "폐업과 창업이 함께 일어나 순 업소 수는 그대로인" 상권이 드러나지 않는다. 원본 확인 결과 **폐업한 자리에 같은 업종의 다른 업소가 들어오면 상가업소번호가 새로 발급된다**(같은 자리 재점유 검증: 한 회차에 사라진 번호의 64%가 같은 위치에서 다른 번호로 재등장, 유지된 번호의 상호명 변경은 0.005%). 따라서 번호의 등장·소멸로 진입과 이탈을 나눠 셀 수 있다.

행정동별로 회차마다 다음을 센다.

| 값 | 정의 |
| --- | --- |
| 이탈 | 직전 회차 그 동에 있던 업소번호가 다음 회차에 사라진 수 |
| 진입 | 다음 회차에 새로 등장한 업소번호 수 |
| 교체율 | 선택 구간 회차별 (진입 + 이탈) 합 ÷ 시작 시점 업소 수 |
| 순증감률 | (진입 − 이탈) 합 ÷ 시작 시점 업소 수 = 6.7의 증감률과 동일 |

회차별 (진입 − 이탈)을 더하면 그 구간의 순증감과 정확히 일치한다(항등식). 교체율은 같은 자리가 구간 안에서 두 번 갈리면 두 번 세므로 100%를 넘을 수 있고, 절대 수치가 아니라 상권 간 상대 비교로 읽는다. 입력은 `data/processed/dong_turnover.csv`다. 이 값은 **동 로컬 기준**이라 다른 동으로 옮긴 소수 사례는 이탈·진입 양쪽에 한 번씩 잡히고, 번호 재발급·자료 누락이 섞일 수 있는 한계는 5.5와 같다.
...
6.7의 증감률은 순 업소 수 차이만 본다. 폐업한 자리에 같은 업종 다른 업소가 들어오면(업소번호는 새로 발급, 5.8) 증감률은 0이지만 상권 안에서는 폐업과 창업이 함께 일어난 것이다. 2025-03~2026-06 회차별 진입·이탈을 더해 교체율(진입+이탈 ÷ 시작 업소 수)로 이 활동량을 본다.

**대전 전체:** 시작 77,904개 대비 누적 이탈 19,913개·진입 22,713개로 **교체율 54.7%**, 순증은 +2,800개(+3.6%)다. 15개월 동안 진입·이탈을 합치면 시작 업소 수의 절반을 넘는 물량이 오갔지만 순증은 3.6%에 그친다. 행정동 교체율 중앙값은 약 54%, 범위는 30.6%~141.6%다.

**교체율이 높은 상권** (진입·이탈 모두 활발)

| 행정동 | 교체율 | 진입 | 이탈 | 순증감률 | 메모 |
| --- | ---: | ---: | ---: | ---: | --- |
| 서구 월평3동 | 141.6% | 111.9% | 29.7% | +82.2% | base 101개, 신규 진입이 규모를 2배 가까이 키움 |
| 유성구 상대동 | 88.0% | 57.2% | 30.8% | +26.5% | 도안, 진입이 이탈의 약 2배 |
| 중구 목동 | 78.4% | 41.9% | 36.5% | +5.3% | 진입·이탈 모두 큼, 순증은 작음 |
| 중구 부사동 | 69.6% | 40.1% | 29.5% | +10.6% | |
| 대덕구 법1동 | 68.1% | 44.4% | 23.6% | +20.8% | 이탈 적고 진입 많음 |
| 서구 도안동 | 67.7% | 35.8% | 31.9% | +3.9% | base 2,087개, 큰 상권인데 3분의 2가 오감 |

**순증은 거의 없는데 교체율이 높은 상권** (겉으로는 제자리, 안에서 물갈이)

| 행정동 | 교체율 | 진입 | 이탈 | 순증감률 |
| --- | ---: | ---: | ---: | ---: |
| 서구 복수동 | 67.2% | 33.5% | 33.7% | −0.2% |
| 동구 효동 | 66.2% | 33.5% | 32.7% | +0.8% |
| 중구 대흥동 | 63.7% | 32.1% | 31.7% | +0.4% |
| 동구 홍도동 | 62.5% | 32.5% | 30.0% | +2.6% |
| 유성구 온천2동 | 59.5% | 29.6% | 29.9% | −0.3% |
| 유성구 온천1동 | 56.9% | 29.3% | 27.6% | +1.8% |

**교체율이 낮은 상권** (진입·이탈 모두 적어 안정)

| 행정동 | 교체율 | 진입 | 이탈 | 순증감률 |
| --- | ---: | ---: | ---: | ---: |
| 서구 기성동 | 30.6% | 15.3% | 15.3% | ±0.0% |
| 서구 변동 | 31.6% | 16.6% | 15.0% | +1.6% |
| 동구 대청동 | 32.0% | 18.8% | 13.3% | +5.5% |
| 동구 삼성동 | 32.2% | 16.5% | 15.7% | +0.8% |
| 대덕구 대화동 | 32.5% | 16.1% | 16.4% | −0.4% |
| 동구 중앙동 | 36.4% | 17.9% | 18.4% | −0.5% |

**관찰:** 중구 대흥동은 6.7에서 성장 정체(+0.4%)·잔존율 중위(82.1%)로 두드러지지 않았지만, 교체율로 보면 63.7%로 상위권이다. 진입 32.1%와 이탈 31.7%가 거의 맞물려 순증만 +8개다. 공급 밀도 상·하위인 중앙동(36.4%)과 월평3동(141.6%)이 교체율 양 극단에 있고, 6.5에서 잔존율이 가장 안정적이던 기성동·삼성동·대화동은 교체율도 가장 낮다.

**해석:** 교체율은 잔존율의 뒷면(이탈)에 진입을 더한 값이라, "기존 업소가 남았나"와 "새 업소가 얼마나 들어왔나"를 함께 본다. 대흥동·복수동·효동처럼 순증이 0에 가까운데 교체율이 높은 상권은 **폐업과 창업이 맞물려 겉으로만 제자리인 곳**으로, 순 업소 수만 보면 놓친다. 다만 이 값도 개폐업일자가 아니라 번호의 등장·소멸로 추정하므로 번호 재발급·자료 누락(5.5)이 섞여 있고, 업종 전환 재점유(음식점→소매)와 동종 교체를 구분하지 않는다.

### 6.9 인터랙티브 대시보드

[인터랙티브 대시보드 열기](https://frost0313z.github.io/daejeon-commercial-analysis/) (로컬: `interactive-dashboard.html`)
...
- 점포 교체율: 선택 구간의 순증감률 × 교체율(진입+이탈) 산점도. 순증은 0 근처인데 위쪽이면 폐업·창업이 맞물린 상권, 교체율 상위·하위 요약 포함(6.8)
...
8. **"제자리"인 상권도 안에서는 크게 갈린다.** 폐업 자리에 새 업소가 들어오면 업소번호가 새로 발급돼 순 업소 수만 보면 활동이 없는 것처럼 보인다. 진입·이탈로 나눠 보면 대전 전체 15개월 교체율은 54.7%이고, 대흥동(63.7%)·복수동(67.2%)·효동(66.2%)은 순증이 0에 가까운데도 시작 업소 수의 3분의 2가 오갔다. 반대로 기성동·삼성동·대화동은 교체율이 30% 안팎으로 진입·이탈이 모두 적다. 이 값도 개폐업일자가 아닌 번호의 등장·소멸로 추정한 것이다(6.8).
...
- 점포 교체율(6.8)은 개폐업일자가 아니라 상가업소번호의 등장·소멸로 진입·이탈을 추정한다. 번호 재발급·자료 누락이 섞일 수 있고, 업종 전환 재점유(예: 음식점→소매)와 동종 교체를 구분하지 않으며, 같은 자리가 구간 안에서 여러 번 갈리면 중복 계산된다. 상권 간 상대 비교로만 읽어야 한다.
