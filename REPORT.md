...
[인터랙티브 대시보드 열기](interactive-dashboard.html)에서 대전 전체·자치구·업종별 추이를 전환하고, 막대와 표에서 증감량·증감률을 확인할 수 있다. 2026년 3월 기준 **78,607개 업소의 유효 좌표 전체**를 0.01도 격자 **2,301개**로 집계한 위치 지도도 포함했다. 지도에서 자치구와 업종을 선택하고 파랑·보라·빨강 밀도 체크박스를 켜고 끌 수 있다.

지도는 같은 격자에 속한 업소 수를 원의 크기와 농도로 표현한다. 이는 밀집 위치를 탐색하기 위한 시점 단면이며, 상권 경계나 개별 점포의 정확한 위치를 뜻하지 않는다. OpenStreetMap 배경을 표시하려면 인터넷 연결이 필요하다.
