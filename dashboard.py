...
def spatial_grid() -> list[dict]:
    raw_files = [p for p in ROOT.rglob("*202603.csv") if "processed" not in p.parts]
    if raw_files:
        with raw_files[0].open(encoding="utf-8-sig", newline="") as f:
            counts = Counter((round(float(r["위도"]), 2), round(float(r["경도"]), 2),
                              r["시군구명"], r["상권업종대분류명"])
                             for r in csv.DictReader(f))
        rows = [{"lat": lat, "lon": lon, "district": district,
                 "category": category, "count": count}
                for (lat, lon, district, category), count in sorted(counts.items())]
        write_csv(SPATIAL.name, rows)
        return rows
    with SPATIAL.open(encoding="utf-8-sig", newline="") as f:
