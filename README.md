...
| 2025-03 고정 코호트 잔존율 | 2026-03 기준 전체 **86.39%** (12개월) |
...
- **비교 기간 선택:** 듀얼 손잡이 슬라이더를 드래그해 시작·종료 시점을 골라 핵심 지표·증감 그래프·히트맵·성장 진단·상세 결과를 그 구간 기준으로 다시 계산. 2024년 말 단절 이전은 다루지 않아 슬라이더는 2025-03~2026-06 6개 시점만 담음
- **관측 시점별 선 그래프:** 대전 전체·자치구·업종·행정동을 여러 항목 동시에 겹쳐 비교(6개 시점 전체, 행정동은 자치구를 먼저 선택)
...
| 자료 | 범위 | 출처 |
| --- | --- | --- |
| 상가(상권)정보 | 대전. 원본 2024-03~2026-06 10개 파일·763,839행 중 안정된 6개(2025-03~2026-06) 사용 | [소상공인시장진흥공단](https://www.data.go.kr/data/15083033/fileData.do) |
...
- **고정 코호트 잔존율:** 2025년 3월 수록 업소 중 이후 같은 번호가 남은 비율
...
전체 재현에 필요한 상가 기준월은 다음 10개입니다. 분석(지표·성장 진단·대시보드)은 이 중 2025-03 이후 6개만 쓰지만, `analysis.ipynb`와 `quality_diagnostics.py`의 수집 품질 진단은 2024년 4개 시점까지 함께 읽어 단절을 확인합니다.
...
| 파일 | 설명 |
| --- | --- |
| [dong_panel.csv](data/processed/dong_panel.csv) | 82개 동 × 10개 업종 × 6개 시점 = 4,920행 균형 패널 |
| [dong_population.csv](data/processed/dong_population.csv) | 행정동별 6개 시점의 인구·연령 구조 |
...
| `images/10_dong_lq_heatmap.png` | 82개 동 × 10개 업종 LQ (2026년 3월) |
...
- 행정동 합계와 기존 자치구 패널의 30개 시점×자치구 조합 전수 일치
- 행정동·인구·경계 코드 **82/82 일치**
- 균형 패널 **4,920행**, 중복 키 0건
- 10개 업종의 업소 수 가중 LQ 평균 1.0
- HHI 0.1~1.0 범위와 고정 코호트 최초 시점(2025-03) 100% 확인
...
- 2024년 말 수집범위 단절 때문에 2024년 4개 시점은 분석에서 제외했고, 남은 2025-10·2026-06에도 소규모 지연 편입 흔적이 있습니다.
